...
echo "🧪 Running Keploy Tests with Your Custom Database..."
...
# Run tests with Keploy using your custom database
docker run --name keploy-v2 -p 16789:16789 --network keploy-network --privileged --pid=host -v $(pwd):$(pwd) -w $(pwd) -v /sys/fs/cgroup:/sys/fs/cgroup -v /sys/kernel/debug:/sys/kernel/debug -v /sys/fs/bpf:/sys/fs/bpf -v /var/run/docker.sock:/var/run/docker.sock --rm -e DATABASE_URL="$DATABASE_URL" -e NODE_ENV="$NODE_ENV" ghcr.io/keploy/keploy test -c "npm start" --delay 10
